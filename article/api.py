...
article_router = Router(tags=['Articles'])
article_service = ArticleService()


@article_router.post("/create_article", response=ArticleResponseSchema, auth=JWTAuth())
def create_article(request, data: ArticleSchema):
    article = article_service.create_article(request.auth["user_id"], data)
    return article


@article_router.put("/update_article/{article_id}", response=ArticleResponseSchema, auth=JWTAuth())
...
@article_router.delete("/delete_article/{article_id}", auth=JWTAuth())
...
@article_router.get("/get_article/{article_id}", response=ArticleResponseSchema)
